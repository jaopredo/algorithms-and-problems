...
def question_4() -> None:
    ...
    ESPACO DO ALUNO:
    (V) Se um problema A pode ser reduzido a um problema B em tempo polinomial, e B
        está em P, então A também está em P.
    (F) Todo problema NP-Difícil é necessariamente NP-Completo.
    (V) Se encontrarmos um algoritmo polinomial para um problema NP-Completo, isso
        prova que P = NP.
    (F) Se tivermos um problema A que é NP-Completo e um problema B que é NP-Difícil,
        e A pode ser reduzido a B em tempo polinomial, então B é necessariamente
        NP-Completo.
    (V) Um problema pode ser simultaneamente NP-Completo e estar em P apenas se P = NP.
    """
    pass
